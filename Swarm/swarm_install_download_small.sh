...
while [ ! -f /var/scratch/sroobol/hash/small_hash.txt ]; do
   sleep 2
   echo "${1}: waiting for hash" 
done 

SMALL_HASH=$( { $COMMANDDIR/swarm up /var/scratch/sroobol/files/small_file.txt; } 2>&1 ) 

for i in `seq 1 50`; 
do
    echo "Start downloading small file"
    OUTPUT=$( { time $COMMANDDIR/swarm down bzz://$SMALL_HASH /dev/null; } 2>&1 )
    echo "$OUTPUT" >> /home/sroobol/output/small_file_download_$i.txt
    echo "Downloaded the small file $i times" 
done
echo "Small file done"
