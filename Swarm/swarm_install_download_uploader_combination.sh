...
echo "${1}: uploading combination file ..." 
OUTPUT=$( $COMMANDDIR/swarm --recursive up /var/scratch/sroobol/files/combination ) 
echo "${OUTPUT}" > /var/scratch/sroobol/hash/combination_hash.txt 
echo "${1}: uploaded combination file hash" 

while [ ! -f /home/sroobol/output/combination_file_download_50.txt ]; 
do
   sleep 2 
done
